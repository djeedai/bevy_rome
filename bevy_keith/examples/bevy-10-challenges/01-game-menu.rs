...
fn draw_button(
    ...
    let brush = if contains {
        if mouse_state.pressed {
            ctx.solid_brush(Color::srgb(0.8, 0.8, 0.8))
        } else {
...
fn run(
    ...
    let color = Color::hsl((time.elapsed_secs() / 3.).fract() * 360., 0.5, 0.5);
    let text = ctx
    ...
    ) && mouse_state.pressed
    {
        ...
        ctx.draw_image(rect, menu_state.image.clone(), ImageScaling::Uniform(1.));
    }
    let rect = Rect {
    ...
    if draw_button(
        ...
        let brush = ctx.solid_brush(Color::srgb(0.8, 0.8, 0.8));
